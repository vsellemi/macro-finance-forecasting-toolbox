...
%% FORECASTING FRED-MD WITH COMPLETE SUBSET REGRESSION - ELLIOTT, GARGANO, TIMMERMANN (2013)
...
% estimate all possible models with k predictors
kmax = 2; 

% index all possible combinations of hyperparameters
[a,b,c,d] = ndgrid(1:py_max,1:pf_max,nf_max:nf_max,1:kmax);
combinations = [reshape(a,[],1), reshape(b,[],1), ...
    reshape(c,[],1),reshape(d,[],1)];


%% CSR MODEL - OPTIMIZE OVER HYPERPARAMETERS
...
CSR_errors = e1;
if isfile('FORECAST_ERRORS.mat'), save('FORECAST_ERRORS.mat', 'CSR_errors','-append'); end
