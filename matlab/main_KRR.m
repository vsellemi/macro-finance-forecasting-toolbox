...
%% FORECASTING FRED-MD WITH KERNEL RIDGE REGRESSION
...
[a,b,c,d,e] = ndgrid(1:length(lambda_vec),1:length(sigma_vec),1:py_max,1:pf_max,nf_max:nf_max);
...
for i = 1:size(combinations,1)
    ...
    switch model_selection
        ...
        case 'ictr'
            ...
            yhat = forecast_KRR(alpha_tr, Xtr, Xtr, sigma);
        ...
        case 'pooscv'
            ...
            for t = 1:length(val_ind)
                ...
                Xte = Xte(end,:);
                ythat = forecast_KRR(alpha_tr, Xtr, Xte, sigma);
        ...
        case 'kfcv'
            ...
            for k = 1:K
                ...
                yhat = forecast_KRR(alpha_tr, Xtr, Xte, sigma);
...
%% KRR MODEL: IN-SAMPLE CRITERIA, PSEUDO-OOS FIT AND EVALUATION

% set hyperparameters based on best validation performance
lambda = lambda_vec(combinations(best_i,1));
sigma  = sigma_vec(combinations(best_i,2));
...
if roll
    ...
    for t = 1:length(test_ind)
        ...
        % estimate KRR on rolling window
        [Xtr,Ytr] = add_lags(YY(temp_ind),factors(temp_ind,1:nf),pf,h,0,py);
        alpha_tr = train_KRR(Xtr,Ytr,lambda,sigma);
...
KRR_errors = e1;
if isfile('FORECAST_ERRORS.mat'), save('FORECAST_ERRORS.mat', 'KRR_errors','-append'); end
