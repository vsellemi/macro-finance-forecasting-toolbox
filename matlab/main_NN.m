...
%% FORECASTING FRED-MD WITH NEURAL NETWORKS
%  author: Victor Sellemi
%  NOTE: requires MATLAB version R2021a+
...
[a,b,c,d,e] = ndgrid(1:hidden_dim_max,1:nlayers_max,1:py_max,1:pf_max,nf_max:nf_max);
...
%% NN MODEL - OPTIMIZE OVER HYPERPARAMETERS
...
for i = 1:size(combinations,1)
    ...
    % set hyperparameters
    hidden_dim = combinations(i,1);
    nlayers    = combinations(i,2);
    py         = combinations(i,3);
    pf         = combinations(i,4);
    nf         = combinations(i,5);
    LayerSizes = repmat(hidden_dim,[1,nlayers]);
    
    % estimate model on in-sample part
    [Xtr,Ytr] = add_lags(YY(train_ind),factors(train_ind,1:nf),pf,h,0,py);
    NN_tr     = fitrnet(Xtr,Ytr,'Standardize',true,'LayerSizes', LayerSizes);
    
    % evaluate the model on the validation set
    [Xval,Yval] = add_lags(YY(val_ind),factors(val_ind,1:nf),pf,h,0,py);
    yhat = predict(NN_tr,Xval);
    ...
    switch model_selection
        ...
        case 'ictr'
            ...
            % evaluate the model on the validation set
            [Xval,Yval] = add_lags(YY(val_ind),factors(val_ind,1:nf),pf,h,0,py);
            yhat = predict(NN_tr,Xtr);
            
            insampIC(i,:) = IC(Ytr,yhat, length(Yval), size(Xval,2));
        ...
        case 'pooscv'
            ...
            for t = 1:length(val_ind)
                ...
                % estimate NN on poos expanding window
...
%% NN MODEL: IN-SAMPLE CRITERIA, PSEUDO-OOS FIT AND EVALUATION

% set hyperparameters based on best validation performance
hidden_dim = combinations(best_i,1);
nlayers    = combinations(best_i,2);
...
if roll
    ...
    for t = 1:length(test_ind)
        ...
        % estimate NN on expanding window
        [Xtr,Ytr] = add_lags(YY(temp_ind),factors(temp_ind,1:nf),pf,h,0,py);
        NN_tr     = fitrnet(Xtr,Ytr,'Standardize',true,'LayerSizes', LayerSizes);
...
NN_errors = e1;
if isfile('FORECAST_ERRORS.mat'), save('FORECAST_ERRORS.mat', 'NN_errors','-append'); end
