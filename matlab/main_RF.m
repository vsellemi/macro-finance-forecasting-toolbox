...
%% FORECASTING FRED-MD WITH REGRESSION TREE ENSEMBLES / BOOSTED OR BAGGED RANDOM FORESTS
...
%     method     = bootstrap aggregation ('Bag') or least-squares boosting ('LSBoost')
%     numCycles  = number of ensemble learning cycles (the more the better but keep in mind computational constraints)
%     learner    = specifications of the weak learner to use in ensemble
%     resamp_ind = should we resample training observations
%     replac_ind = should we sample with replacement
method         = 'Bag';
numCycles_max  = 100;
resamp_ind     = 'on';
replac_ind     = 'off';
%learner    = templateTree('MaxNumSplits',5);

% for reproducibility
rng(1);

% index all possible combinations of hyperparameters
[a,b,c,d] = ndgrid(numCycles_max,1:py_max,1:pf_max,nf_max:nf_max);
...
%% RF MODEL - OPTIMIZE OVER HYPERPARAMETERS
...
for i = 1:size(combinations,1)
    ...
    switch model_selection
        ...
        case 'icval'
            ...
            RF_tr     = fitrensemble(Xtr,Ytr,'Method',method, 'NPrint', 'off', 'NumLearningCycles', numCycles);
            
            % in-sample evaluation on validation set
            [Xval,Yval] = add_lags(YY(val_ind),factors(val_ind,1:nf),pf,h,0,py);
            yhat = predict(RF_tr,Xval);
        ...
        case 'pooscv'
            ...
            for t = 1:length(val_ind)
                ...
                % estimate RF on poos expanding window
...
%% RF MODEL: IN-SAMPLE CRITERIA, PSEUDO-OOS FIT AND EVALUATION

% set hyperparameters based on best validation performance
numCycles = combinations(best_i,1);
...
if roll
    ...
    for t = 1:length(test_ind)
        ...
        % estimate RF on expanding window
        [Xtr,Ytr] = add_lags(YY(temp_ind),factors(temp_ind,1:nf),pf,h,0,py);
        RF_tr     = fitrensemble(Xtr,Ytr,'Method',method, 'NPrint', 'off', 'NumLearningCycles', numCycles);
...
RF_errors = e1;
if isfile('FORECAST_ERRORS.mat'), save('FORECAST_ERRORS.mat', 'RF_errors','-append'); end
