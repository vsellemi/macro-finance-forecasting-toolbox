...
%% FORECASTING FRED-MD WITH SUPPORT VECTOR REGRESSION
...
%     kernel = kernel function (gaussian is used in Coulombe et al. (2019))
eps_max = .1;
C_max   = 0.5;
sigma_max  = 2;
kernel = 'gaussian';

% set vector of options for non-integer hyperparameters
eps_vec    = linspace(0,eps_max,5);
C_vec      = linspace(0.1,C_max,5);
sigma_vec  = linspace(0.1,sigma_max,5);

% index all possible combinations of hyperparameters
[a,b,c,d,e,f] = ndgrid(1:length(eps),1:length(C_vec),1:length(sigma_vec),...
    1:py_max,1:pf_max,nf_max:nf_max);
combinations = [reshape(a,[],1), reshape(b,[],1), reshape(c,[],1), ...
    reshape(d,[],1),reshape(e,[],1),reshape(f,[],1)];

disp(join(['Testing ', num2str(size(combinations,1), "%.0f"), ' combinations of hyperparameters.']))


%% SVR MODEL - OPTIMIZE OVER HYPERPARAMETERS
...
for i = 1:size(combinations,1)
    ...
    % set hyperparameters
    eps    = eps_vec(combinations(i,1));
    C      = C_vec(combinations(i,2));
    sigma  = sigma_vec(combinations(i,3));
    py     = combinations(i,4);
    pf     = combinations(i,5);
    nf     = combinations(i,6);
...
%% SVR MODEL: IN-SAMPLE CRITERIA, PSEUDO-OOS FIT AND EVALUATION

% set hyperparameters based on best validation performance
eps    = eps_vec(combinations(best_i,1));
C      = C_vec(combinations(best_i,2));
sigma  = sigma_vec(combinations(best_i,3));
py     = combinations(best_i,4);
pf     = combinations(best_i,5);
nf     = combinations(best_i,6);
p      = max(pf,py);
...
if roll
    ...
    for t = 1:length(test_ind)
        ...
        % estimate SVR on rolling window
        [Xtr,Ytr] = add_lags(YY(temp_ind),factors(temp_ind,1:nf),pf,h,0,py);
        SVR_tr = train_SVR(Xtr,Ytr,sigma,kernel,eps,C);
    ...
    for t = 1:length(test_ind)
        ...
        % estimate SVR on expanding window
        [Xtr,Ytr] = add_lags(YY(temp_ind),factors(temp_ind,1:nf),pf,h,0,py);
        SVR_tr = train_SVR(Xtr,Ytr,sigma,kernel,eps,C);
...
SVR_errors = eval.errors;
if isfile('FORECAST_ERRORS.mat'), save('FORECAST_ERRORS.mat', 'SVR_errors','-append'); end
