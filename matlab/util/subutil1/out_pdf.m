function [x,y]=out_pdf(X,type,nb,range,bw)
% PURPOSE: Gives the output for an univariate kernel density plot
% 
% USAGE:
%     [x,y]=out_pdf(X,nb,range,bw)
% 
% INPUTS:
...
% COMMENTS:
%
% modified by DP after k_pdf by François Desmoulins-Lebeault

[N,p]=size(X);
if p~=1
    error('must input a single series');
end
