function [result,res_for_xls] = coda(draws,vnames,info,fid)
...
%          result(i).auto20= autocorrelation at lag 20
...
cnames = strvcat(lstring1,lstring2,lstring3,lstring4,lstring5);
rnames = vstring;
for i=1:nvar
...
mprint(aprt,in);
res_for_xls.autocorr = [[cellstr(in.rnames(1,:));cellstr(in.cnames)]';[cellstr(in.rnames(2:end,:)) num2cell(aprt)]];
...
mprint(rout,in2);
res_for_xls.raftery = [[cellstr(in2.rnames(1,:));cellstr(in2.cnames)]';[cellstr(in2.rnames(2:end,:)) num2cell(rout)]];
...
mprint(gout,in3);
res_for_xls.geweke_conv_iid = [[cellstr(in3.rnames(1,:));cellstr(in3.cnames)]';[cellstr(in3.rnames(2:end,:)) num2cell(gout)]];
...
in4.rnames = rnames;
mprint(gout2,in4);
res_for_xls.geweke_conv_spect = [[cellstr(in4.rnames(1,:));cellstr(in4.cnames)]';[cellstr(in4.rnames(2:end,:)) num2cell(gout2)]];
...
mprint(gout3,in);
res_for_xls.geweke_test = [res_for_xls.geweke_test;...
    [Vname{i},{''},{''},{''}];...
    [{''},{''},{''},{''}];...
    [[cellstr(in.rnames(1,:));cellstr(in.cnames)]';[cellstr(in.rnames(2:end,:)) num2cell(gout3)]];...
    [{''},{''},{''},{''}]];
end;
